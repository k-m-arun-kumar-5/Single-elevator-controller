...
/* ----------------------------- global function declaration -------------------- */
 static uint8_t Dev_Oper_Read_Status_Proc( const uint8_t read_dev_id, const uint8_t rcvd_read_status, unsigned char last_read_char);
 static uint8_t Cur_Data_ID_Dev_Src_Basic_Oper_Func(const uint8_t dev_src_basic_oper_func, const uint8_t dev_type, const dev_id_t *const dev_basic_oper_func_ptr);
 static uint8_t Data_ID_Read_Try_Complete_Proc(const uint8_t data_id);
...
Dev_Oper_Func_t Dev_Oper_Func_Table[] = { 
						...
						{ Seg7_DeInit, Seg7_Init, Seg7_No_Access, Seg7_Allow_Access, Seg7_Disable, Seg7_Enable, NULL_PTR, NULL_PTR, NULL_PTR, Seg7_Write },
                         ...
                         {UART_DeInit, UART_Init, UART_No_Access, UART_Allow_Access, UART_Disable, UART_Enable, UART_Receive_Char, UART_Transmit, NULL_PTR, NULL_PTR},
                        #else
                         { NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR},
                        #endif
...
BUGS           :    
-*------------------------------------------------------------*/
uint8_t Cur_Data_ID_Read_Oper_Func(const uint8_t dev_type, unsigned char *const last_read_data_char_ptr)
{
	...
	uint8_t ret_status, i = 0, max_num_read_devs, min_read_dev_id, max_read_dev_id, read_data_stream_fmt; 	
	...
	*last_read_data_char_ptr = NULL_CHAR;
	...
	{
		case DEV_INPUT_TYPE:
		    if((ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_INPUT_DATA_STREAM, &read_data_stream_fmt)) != SUCCESS)
			{
		...
		case DEV_RECEIVE_TYPE:
		    if((ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_RECEIVE_DATA_STREAM, &read_data_stream_fmt)) != SUCCESS)
			{
				  system_status_flag = ERR_APPL_DATA_RETRIEVE;
  ...
  /*  if(read_data_stream_fmt == DATA_NO_STREAM)
	{
		system_status_flag = CUR_DATA_ID_NO_STREAM;
		Error_or_Warning_Proc("08.01.06", ERROR_OCCURED, system_status_flag);
        return system_status_flag;
	} */
	while(i < max_num_read_devs && (data_dev_read_oper_func + i)->dev_id >= min_read_dev_id && (data_dev_read_oper_func + i)->dev_id <= max_read_dev_id  )
	{
		if(cur_data_id_status_para_ptr->data_read_status.max_try_reached_flag == STATE_YES)
	    {
	    	return CUR_DATA_ID_READ_MAX_TRIED;
	   ...
	   else
	   {
		...
		switch(dev_type)
		{
			case DEV_INPUT_TYPE:
	           ...
	           ret_status = (*Dev_Oper_Func_Table[(data_dev_read_oper_func + i)->dev_id].Dev_Read_Func_Ptr)((data_dev_read_oper_func + i)->dev_ch_id, &dev_read_ctrl_para);
	        break;
			case DEV_RECEIVE_TYPE:
			   if(Dev_Oper_Func_Table[(data_dev_read_oper_func + i)->dev_id].Dev_Receive_Func_Ptr == NULL_PTR)
	           {
		          system_status_flag = ERR_NULL_PTR;
...
static uint8_t Dev_Oper_Read_Status_Proc( const uint8_t read_dev_id, const uint8_t rcvd_read_status, unsigned char last_read_char)
{
	...
	uint8_t data_valid_read_char_flag = STATE_NO, ret_status = rcvd_read_status, temp_ret_status, input_data_type, char_type_read_enable_flag, dev_type;
	...
	{ 
		...
		case  COMM_RCVD_CHAR:
		   ...
		   {
			   system_status_flag = ERR_INVALID_DATA_CHAR_READ;
		   ...
		   switch( cur_data_id_status_para_ptr->data_read_status.data_read_mode) 
	       {
			 /* if data's max data allocated char < num entered chars, then new entered chars are taken, till terminator char is read.
			   if data's max data allocated char >= num entered chars, then newly read chars are discarded, till terminator char is read */
		      case DATA_ID_READ_MODE_TILL_TERMINATOR:
			     if(( temp_ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_DATA_READ_TERMINATOR_CHAR, &data_read_terminator_char)) != SUCCESS)
				 {
					  system_status_flag = ERR_APPL_DATA_RETRIEVE;
		  ...
		  {  
	         //numeric char read
	         if((temp_ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_READ_NUM_CHAR_ENABLE_FLAG, &char_type_read_enable_flag)) != SUCCESS)
			 {				 
			      system_status_flag = ERR_APPL_DATA_RETRIEVE;
		  ...
		  {
			  if((temp_ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_READ_SMALL_ENGLISH_ALPHA_CHAR_ENABLE_FLAG, &char_type_read_enable_flag)) != SUCCESS)
			  {
			      system_status_flag = ERR_APPL_DATA_RETRIEVE;
		  ...
		  {
			  if((temp_ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_READ_BIG_ENGLISH_ALPHA_CHAR_ENABLE_FLAG, &char_type_read_enable_flag)) != SUCCESS)
			  {
				  system_status_flag = ERR_APPL_DATA_RETRIEVE;
		  ...
		  if(last_read_char >= BEGIN_CTRL_CHARS_ASCII_CODE && last_read_char <= END_CTRL_CHARS_ASCII_CODE)
		  {
			  switch(last_read_char)
			   {
			       case BACKSPACE_CHAR:
					  ...
					  {
                          case DEV_INPUT_TYPE:
                             if((temp_ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_INPUT_DATA_TYPE, &input_data_type)) != SUCCESS)
							 {
							    system_status_flag = ERR_APPL_DATA_RETRIEVE;
			  ...
			  /* if((temp_ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_READ_CTRL_CHAR_ENABLE_FLAG, &char_type_read_enable_flag)) != SUCCESS)
			     {
			         system_status_flag = ERR_APPL_DATA_RETRIEVE;
		  ...
		  {
			  if((temp_ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_READ_SPECIAL_CHAR_ENABLE_FLAG, &char_type_read_enable_flag)) != SUCCESS)
			  {
			      system_status_flag = ERR_APPL_DATA_RETRIEVE;
	      ...
	      for(j = 0; j < max_data_dev_type_srcs; ++j)
		  {
	         ...
	         while(i < max_data_dev_type_srcs && (data_dev_basic_oper_func_ptr + i)->dev_id >= min_dev_id && (data_dev_basic_oper_func_ptr + i)->dev_id <= max_dev_id )
	         {
                 if((cur_data_devs_src_allow_ptr + j )->dev_id == (data_dev_basic_oper_func_ptr + i)->dev_id )
			     {
				      ...
				      {
					      if((ret_status = Cur_Data_ID_Dev_Src_Basic_Oper_Func(dev_src_basic_oper_func, dev_type, data_dev_basic_oper_func_ptr + i)) != SUCCESS)
						  {
							  system_status_flag = ERR_DEV_SRC_BASIC_OPER_FUNC_INVALID;
...
BUGS           :    
-*------------------------------------------------------------*/
uint8_t Cur_Data_ID_Write_Oper_Func(const uint8_t dev_type, const dev_write_ctrl_para_t *const dev_write_ctrl_para_ptr)
{
	data_write_ctrl_ptr_t data_write_ctrl;
    const dev_id_t *data_dev_write_oper;
	uint8_t ret_status, i = 0, output_data_type, max_num_write_devs, min_write_dev_id, max_write_dev_id, write_data_stream_fmt;
	
    if(dev_write_ctrl_para_ptr == NULL_PTR)
	{
		system_status_flag = ERR_NULL_PTR;
	...
	{	
	   case DEV_OUTPUT_TYPE:
		  ...
		  if((ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_OUTPUT_DATA_STREAM, &write_data_stream_fmt)) != SUCCESS)
		  {
				  system_status_flag = ERR_APPL_DATA_RETRIEVE;
	   ...
	   break;
       case DEV_TRANSMIT_TYPE:
	      if((ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_TRANSMIT_DATA_STREAM, &write_data_stream_fmt)) != SUCCESS)
		  {
				  system_status_flag = ERR_APPL_DATA_RETRIEVE;
 ...
 // To be called when data stream is supported
  /*  if(write_data_stream_fmt == DATA_NO_STREAM)
	{
		system_status_flag = CUR_DATA_ID_NO_STREAM;
	...
	while(i < max_num_write_devs && (data_dev_write_oper + i)->dev_id >= min_write_dev_id && (data_dev_write_oper + i)->dev_id <= max_write_dev_id )
	{
		switch(dev_type)
		{
			case DEV_OUTPUT_TYPE:
	          ...
	          ret_status = (*Dev_Oper_Func_Table[(data_dev_write_oper + i)->dev_id].Dev_Write_Func_Ptr)((data_dev_write_oper + i)->dev_ch_id, &data_write_ctrl);
		   break;
           case DEV_TRANSMIT_TYPE:
		      if(Dev_Oper_Func_Table[(data_dev_write_oper + i)->dev_id].Dev_Transmit_Func_Ptr == NULL_PTR)
	          {
		         system_status_flag = ERR_NULL_PTR;
...
static uint8_t Cur_Data_ID_Dev_Src_Basic_Oper_Func(const uint8_t dev_src_basic_oper_func, const uint8_t dev_type, const dev_id_t *const dev_basic_oper_func_ptr)
{
	uint8_t ret_status, comm_dev_func = 0, comm_dev_id_offset;
    ...
    switch(dev_src_basic_oper_func)
	{
		 ...
		 case COMM_DEV_DEINIT_OPER_FUNC:
		    ...
		    if((ret_status = (*Dev_Oper_Func_Table[dev_basic_oper_func_ptr->dev_id].Dev_DeInit_Func_Ptr)(dev_basic_oper_func_ptr->dev_ch_id, DEV_DEINIT_OPER)) != SUCCESS)
		    {
		 ...
		 case COMM_DEV_INIT_OPER_FUNC:
		     ...
		     if((ret_status = (*Dev_Oper_Func_Table[dev_basic_oper_func_ptr->dev_id].Dev_Init_Func_Ptr)(dev_basic_oper_func_ptr->dev_ch_id, DEV_INIT_OPER)) != SUCCESS)
			 {
		...
		case OUTPUT_DEV_DISABLE_OPER_FUNC:		
		   ...
		   if((ret_status = (*Dev_Oper_Func_Table[dev_basic_oper_func_ptr->dev_id].Dev_Disable_Func_Ptr)(dev_basic_oper_func_ptr->dev_ch_id)) != SUCCESS)
		   {
            ...
            if(comm_dev_func == 0x01)
            {
			  ...
			  {
			     case DEV_RECEIVE_TYPE:
			       if(Comm_Dev_Spec_Oper_Enable_Ctrl_Func_Table[comm_dev_id_offset].Comm_Dev_Receive_Func_Ptr == NULL_PTR)
				   {
					   system_status_flag = ERR_NULL_PTR;
				       Error_or_Warning_Proc("08.08.12", ERROR_OCCURED, system_status_flag);
                       return system_status_flag;
				   }
				   if((ret_status = (*Comm_Dev_Spec_Oper_Enable_Ctrl_Func_Table[comm_dev_id_offset].Comm_Dev_Receive_Func_Ptr)(dev_basic_oper_func_ptr->dev_ch_id, STATE_NO)) != SUCCESS)
		           {
			             system_status_flag = ERR_DEV_OPER_DISABLE_FUNC;
				 ...
				 break;
				 case DEV_TRANSMIT_TYPE:
			       if(Comm_Dev_Spec_Oper_Enable_Ctrl_Func_Table[comm_dev_id_offset].Comm_Dev_Transmit_Func_Ptr == NULL_PTR)
				   {
					   system_status_flag = ERR_NULL_PTR;
				       Error_or_Warning_Proc("08.08.14", ERROR_OCCURED, system_status_flag);
                       return system_status_flag;
				   }
				   if((ret_status = (*Comm_Dev_Spec_Oper_Enable_Ctrl_Func_Table[comm_dev_id_offset].Comm_Dev_Transmit_Func_Ptr)(dev_basic_oper_func_ptr->dev_ch_id, STATE_NO)) != SUCCESS)
		           {
			             system_status_flag = ERR_DEV_OPER_DISABLE_FUNC;
           ...
           if(comm_dev_func == 0x03)
           {
			  ...
			  switch(dev_type)
			  {
			     case DEV_RECEIVE_TYPE:
				   ...
				   if((ret_status = (*Comm_Dev_Spec_Oper_Enable_Ctrl_Func_Table[comm_dev_id_offset].Comm_Dev_Receive_Func_Ptr)(dev_basic_oper_func_ptr->dev_ch_id, STATE_YES)) != SUCCESS)
		           {
			             system_status_flag = ERR_DEV_OPER_DISABLE_FUNC;
				         Error_or_Warning_Proc("08.08.13", ERROR_OCCURED, system_status_flag);
			             return system_status_flag;
				 ...
				 case DEV_TRANSMIT_TYPE:
				   ...
				   if((ret_status = (*Comm_Dev_Spec_Oper_Enable_Ctrl_Func_Table[comm_dev_id_offset].Comm_Dev_Transmit_Func_Ptr)(dev_basic_oper_func_ptr->dev_ch_id, STATE_YES)) != SUCCESS)
		           {
			             system_status_flag = ERR_DEV_OPER_DISABLE_FUNC;
				         Error_or_Warning_Proc("08.08.15", ERROR_OCCURED, system_status_flag);
			             return system_status_flag;
...
{
	data_id_status_para_t *cur_data_id_status_para_ptr = NULL_PTR;
	uint8_t max_num_data_read_try, ret_status;	
	...
	#endif
	if((ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_MAX_NUM_DATA_READ_TRY, &max_num_data_read_try)) != SUCCESS)
	{
