...
volatile timer_or_counter_status_t timer_or_counter_status[NUM_TIMER_DEV_ID_CHS];
...
{
	volatile timer_or_counter_status_t *cur_timer_or_counter_status_ptr;
	volatile timer_or_counter_ctrl_t  *cur_timer_or_counter_ctrl_ptr;
	uint16_t ret_status, timeout_type, actual_timeout_in_ms_or_max_count, timer_min_timeout_in_ms;
	...
	cur_timer_or_counter_ctrl_ptr = timer_or_counter_ctrl + run_timer_run_id;
	cur_timer_or_counter_status_ptr = timer_or_counter_status + timer_ch_id;
	if(cur_timer_or_counter_ctrl_ptr->match_or_capture_id >= NUM_MATCH_REGS)
	{
		 system_status_flag = ERR_DEV_CH_ID_EXCEEDS;
		 Error_or_Warning_Proc("12.01.03", ERROR_OCCURED, system_status_flag);
		 return system_status_flag;
	}
	cur_timer_or_counter_status_ptr->timer_or_counter_last_run_id = cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id =  run_timer_run_id;
 	if(cur_timer_or_counter_ctrl_ptr->mode_type == TIMER_MODE)
	{
		    switch(timer_ch_id)
		    {
			 ...
			 timer_remainder = cur_timer_or_counter_ctrl_ptr->conf_timeout_in_ms_or_max_count % timer_min_timeout_in_ms;			 
		     if(timer_remainder == 0)
		     {
		      	actual_timeout_in_ms_or_max_count = cur_timer_or_counter_ctrl_ptr->conf_timeout_in_ms_or_max_count;
		     } 
             else
		     {			 
		          // set_timer_req_time_delay_in_ms is not in multiples of timer_min_timeout_in_ms
		            ...
		            actual_timeout_in_ms_or_max_count = cur_timer_or_counter_ctrl_ptr->conf_timeout_in_ms_or_max_count - timer_remainder;
                     ...
                     actual_timeout_in_ms_or_max_count =  cur_timer_or_counter_ctrl_ptr->conf_timeout_in_ms_or_max_count + timer_min_timeout_in_ms - timer_remainder ;	
                  #endif	
		     }
			 max_count_per_timeout = actual_timeout_in_ms_or_max_count / timer_min_timeout_in_ms;
			   ...
			   Print("INFO: To timeout in milli sec : %u \r",uint32_temp_disp_data );
			 #endif
			 ...
			 cur_timer_or_counter_ctrl_ptr->max_count_per_timeout = max_count_per_timeout;
			 cur_timer_or_counter_ctrl_ptr->actual_timeout_in_ms_or_max_count = actual_timeout_in_ms_or_max_count;
			 cur_timer_or_counter_ctrl_ptr->elapsed_num_timeouts = 0;
			 cur_timer_or_counter_ctrl_ptr->elapsed_count_per_timeout = 0;
			 cur_timer_or_counter_ctrl_ptr->timer_max_num_timeout_occured_flag = STATE_NO;
			 ...
			 *timer_mr_ptr[timer_ch_id][cur_timer_or_counter_ctrl_ptr->match_or_capture_id] = timer_min_timeout_in_ms; 			 
             if(cur_timer_or_counter_ctrl_ptr->match_interrupt_or_capture_rising_flag == STATE_YES)
	         {
				  //EVENT_INTERRUPT
              if( cur_timer_or_counter_ctrl_ptr->max_count_per_timeout == 1)
              {     
			  ...
			  *tmr_mcr_ptr[timer_ch_id] |= (1 << (3 * cur_timer_or_counter_ctrl_ptr->match_or_capture_id));	
				 ...
				 if(cur_timer_or_counter_status_ptr->exec_timer_interrupt_enable_proc == STATE_NO)
				 {
				    if((ret_status =  Enable_Interrupt_Src(INTP_REQ_TMR0 + timer_ch_id)) != SUCCESS)
	                {
				    ...
				    cur_timer_or_counter_status_ptr->exec_timer_interrupt_enable_proc = STATE_YES;
				 }
				 cur_timer_or_counter_ctrl_ptr->state = TMR_OR_COUNTER_RUNNING_STATE;
				*tmr_tcr_ptr[timer_ch_id] = (1 << 0);  		  // timer run
	         }
             else
			 {
				// EVENT_POLLING 
          if(cur_timer_or_counter_ctrl_ptr->max_num_timeouts != NO_MAX_NUM_TIMEOUTS_OR_COUNT)  
          {            
        ...
        *tmr_mcr_ptr[timer_ch_id] &= ~(1 << (3 * cur_timer_or_counter_ctrl_ptr->match_or_capture_id));	  
		 ...
		 *tmr_tcr_ptr[timer_ch_id] = (1 << 0);  		  // Enable timer 
            ...
            if(cur_timer_or_counter_ctrl_ptr->max_num_timeouts != NO_MAX_NUM_TIMEOUTS_OR_COUNT)
            {
             	++cur_timer_or_counter_ctrl_ptr->elapsed_num_timeouts;
            }
            (timer_or_counter_interrupt_data + timer_ch_id)->timer_or_counter_run_id = cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id;          
            (timer_or_counter_interrupt_data + timer_ch_id)->elapsed_num_timeouts_or_count = cur_timer_or_counter_ctrl_ptr->elapsed_num_timeouts;
			...
			switch(ret_status)
			{
			    ...
			    default:
		           ...
		           return system_status_flag;
					...
					if(cur_timer_or_counter_ctrl_ptr->max_num_timeouts != NO_MAX_NUM_TIMEOUTS_OR_COUNT)
			        {
					   	if(cur_timer_or_counter_ctrl_ptr->elapsed_num_timeouts == cur_timer_or_counter_ctrl_ptr->max_num_timeouts - 1)
				    	{
					         *tmr_mcr_ptr[timer_ch_id] &= ~(1 << (2 + (3 * cur_timer_or_counter_ctrl_ptr->match_or_capture_id)));	
                            // TC and PC will reset and TCR[0]= 0 if MR matches the TC.					
                             *tmr_mcr_ptr[timer_ch_id] |= (1 << (1 + (3 * cur_timer_or_counter_ctrl_ptr->match_or_capture_id)));	
			    ...
			    } while(cur_timer_or_counter_ctrl_ptr->max_num_timeouts == NO_MAX_NUM_TIMEOUTS_OR_COUNT || cur_timer_or_counter_ctrl_ptr->elapsed_num_timeouts < cur_timer_or_counter_ctrl_ptr->max_num_timeouts);
          if(cur_timer_or_counter_ctrl_ptr->max_num_timeouts != NO_MAX_NUM_TIMEOUTS_OR_COUNT) 
          {
               Timer_Stop(timer_ch_id); 
			        cur_timer_or_counter_ctrl_ptr->timer_max_num_timeout_occured_flag = STATE_YES;
          }
		}
	 }
	 return SUCCESS;	
  ...
  volatile timer_or_counter_ctrl_t  *cur_timer_or_counter_ctrl_ptr;
    ...
    *tmr_tcr_ptr[timer_ch_id] = 0; // Stop timer 
	switch(timer_ch_id)
	{
		...
		case CH_ID_01:
            cur_timer_or_counter_status_ptr = timer_or_counter_status + timer_ch_id;
            if(cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id == TMR_OR_COUNTER_STOP_ID)
            {
			    return SUCCESS;
            }
			if(cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id == TMR_OR_COUNTER_PAUSE_ID)
			{
				cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id = TMR_OR_COUNTER_STOP_ID;
		        cur_timer_or_counter_ctrl_ptr = timer_or_counter_ctrl + cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id;
                cur_timer_or_counter_status_ptr->timer_or_counter_last_run_id = cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id;
                return SUCCESS;  				
			}
            if( cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id >= NUM_TIMER_AND_COUNTER_IDS )
        	{
	         	 system_status_flag = ERR_TIMER_ID_EXCEEDS;
	        ...
	        cur_timer_or_counter_ctrl_ptr = timer_or_counter_ctrl + cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id;
            cur_timer_or_counter_status_ptr->timer_or_counter_last_run_id = cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id;
			*tmr_ir_ptr[timer_ch_id] = (1 << cur_timer_or_counter_ctrl_ptr->match_or_capture_id);
            *tmr_mcr_ptr[timer_ch_id] &= ~(7 << (3 * cur_timer_or_counter_ctrl_ptr->match_or_capture_id));	           
           cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id = TMR_OR_COUNTER_STOP_ID;		  
		break;       
	}
	 cur_timer_or_counter_ctrl_ptr->state = TMR_OR_COUNTER_STOPPED_STATE;
    return SUCCESS; 
}
...
void ISR_Timer_0(void) __irq
{
	...
	{		
        (src_interrupt_dev_data + src_interrupt_data_arr_free_index)->src_interrupt_dev = INTP_REQ_TMR0;
		...
		(src_interrupt_dev_data + src_interrupt_data_arr_free_index)->src_interrupt_data_ptr = (volatile void *volatile) (timer_or_counter_interrupt_data + CH_ID_00);
	}
...
void ISR_Timer_1(void) __irq
{
	...
	if(cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id == TMR_OR_COUNTER_STOP_ID || cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id == TMR_OR_COUNTER_PAUSE_ID || cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id >= NUM_TIMER_AND_COUNTER_IDS)
	{
		Timer_Stop(CH_ID_01);		 
	}
	else
	{
		...
		{	
            ++cur_timer_or_counter_ctrl_ptr->elapsed_count_per_timeout;
            if(cur_timer_or_counter_ctrl_ptr->elapsed_count_per_timeout >= cur_timer_or_counter_ctrl_ptr->max_count_per_timeout)
			{
				 cur_timer_or_counter_ctrl_ptr->elapsed_count_per_timeout = 0;							
                (src_interrupt_dev_data + src_interrupt_data_arr_free_index)->src_interrupt_dev = INTP_REQ_TMR1;
			    (src_interrupt_dev_data + src_interrupt_data_arr_free_index)->process_interrupt_status = TMR_NO_MAX_NUM_TIMEOUT_YET_PROCESS;
			    (timer_or_counter_interrupt_data + CH_ID_01)->timer_or_counter_run_id = cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id;
			    (timer_or_counter_interrupt_data + CH_ID_01)->elapsed_num_timeouts_or_count = NO_MAX_NUM_TIMEOUTS_OR_COUNT;
			    if(cur_timer_or_counter_ctrl_ptr->max_num_timeouts != NO_MAX_NUM_TIMEOUTS_OR_COUNT)
		        {
			    	++cur_timer_or_counter_ctrl_ptr->elapsed_num_timeouts;
				    (timer_or_counter_interrupt_data + CH_ID_01)->elapsed_num_timeouts_or_count = cur_timer_or_counter_ctrl_ptr->elapsed_num_timeouts;
				    if(cur_timer_or_counter_ctrl_ptr->elapsed_num_timeouts < cur_timer_or_counter_ctrl_ptr->max_num_timeouts - 1)
				    {
                        (src_interrupt_dev_data + src_interrupt_data_arr_free_index)->process_interrupt_status = TMR_BEFORE_LAST_TIMEOUT_YET_PROCESS;				   
                    }
				    else if(cur_timer_or_counter_ctrl_ptr->elapsed_num_timeouts == cur_timer_or_counter_ctrl_ptr->max_num_timeouts - 1)
				    {
				  	   //commented due to timer running infinitely bug, without interrupting on TC match on Match_Register_id and process ISR_Timer_1()	  
				        /* 
						 *tmr_mcr_ptr[timer_ch_id] &= ~(1 << (1 + (3 * cur_timer_or_counter_ctrl_ptr->match_or_capture_id)));
		        ...
		        (src_interrupt_dev_data + src_interrupt_data_arr_free_index)->src_interrupt_data_ptr = (volatile void *volatile)(timer_or_counter_interrupt_data + CH_ID_01);
		     }
		     *tmr_ir_ptr[CH_ID_01] = (1 << cur_timer_or_counter_ctrl_ptr->match_or_capture_id); //Write back to IR to clear Interrupt Flag
	    }
	}		
	VICVectAddr = 0x0; //This is to signal end of interrupt execution
}
...
uint16_t Disable_Timer_Interrupt(const uint8_t timer_ch_id)
{
    ...
    switch(timer_ch_id)
    {
	   case CH_ID_01:
	   	 cur_timer_or_counter_status_ptr = timer_or_counter_status + timer_ch_id;			
         cur_timer_or_counter_status_ptr->exec_timer_interrupt_enable_proc = STATE_NO;
	   break;	
    }
    if((ret_status = Disable_Interrupt_Src(INTP_REQ_TMR0 + timer_ch_id)) != SUCCESS)
    {
...
uint16_t Timer_0_Run(const uint8_t match_ch_id, const uint16_t appl_proc_exec_conf_time_interval_in_ms)
{
	...
	if(match_ch_id >= NUM_MATCH_REGS)
	{
		 system_status_flag = ERR_DEV_CH_ID_EXCEEDS;
		 Error_or_Warning_Proc("12.06.01", ERROR_OCCURED, system_status_flag);
		 return system_status_flag;
	}
	if((ret_status = Timer_Stop(CH_ID_00)) != SUCCESS)
    {
         system_status_flag = ERR_TIMER_STOP_PROC;
	...
	*tmr_mcr_ptr[CH_ID_00] = 0;
     ...
     *tmr_mcr_ptr[CH_ID_00] |= (1 << (3 * timer_0_ctrl.match_ch_id));	
	 if((ret_status =  Enable_Interrupt_Src(INTP_REQ_TMR0 + CH_ID_00)) != SUCCESS)
	 {
	   	    system_status_flag = ERR_ENABLE_INTERRUPT;
...
uint16_t Timer_Interrupt_Retrieve_Data_Arr(const uint8_t timer_ch_id, timer_or_counter_data_t *const retrieved_timer_or_counter_interrupt_data_ptr)
{
	  ...
	  uint8_t intp_req_timer_ch_id, timer_interrupt_data_arr_index;
	  
    if(retrieved_timer_or_counter_interrupt_data_ptr == NULL_PTR)
    {
	  ...
	  {
		  case CH_ID_00:
		    intp_req_timer_ch_id = INTP_REQ_TMR0;
		  break;
		  case CH_ID_01:
		    intp_req_timer_ch_id = INTP_REQ_TMR1;
		  break;
	  }
      ret_status = Search_Src_Interrupt_Data_Arr_By_Src(intp_req_timer_ch_id, &timer_interrupt_data_arr_index);
      switch(ret_status)
	  {
    ...
    *tmr_tcr_ptr[timer_ch_id] = 0; // Stop timer 
	switch(timer_ch_id)
	{
		...
		case CH_ID_01:
			...
			*tmr_ir_ptr[timer_ch_id] = (1 << cur_timer_or_counter_ctrl_ptr->match_or_capture_id);
            *tmr_mcr_ptr[timer_ch_id] &= ~(7 << (3 * cur_timer_or_counter_ctrl_ptr->match_or_capture_id));	           
            cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id = TMR_OR_COUNTER_PAUSE_ID;
		break;       
	}
	cur_timer_or_counter_ctrl_ptr->state = TMR_OR_COUNTER_PAUSED_STATE;
    return SUCCESS; 
}
 ...
 uint16_t Timer_Resume(const uint8_t timer_ch_id, const uint8_t resume_timer_run_id)
{
	...
	cur_timer_or_counter_ctrl_ptr = timer_or_counter_ctrl + resume_timer_run_id;
	if(cur_timer_or_counter_ctrl_ptr->state != TMR_OR_COUNTER_PAUSED_STATE)
	{
	...
	cur_timer_or_counter_status_ptr = timer_or_counter_status + timer_ch_id;
	cur_timer_or_counter_status_ptr->timer_or_counter_last_run_id = cur_timer_or_counter_status_ptr->timer_or_counter_cur_run_id =  resume_timer_run_id;
 	if(cur_timer_or_counter_ctrl_ptr->mode_type == TIMER_MODE)
	{
		    switch(timer_ch_id)
		    {
			 ...
			 *timer_mr_ptr[timer_ch_id][cur_timer_or_counter_ctrl_ptr->match_or_capture_id] = timer_min_timeout_in_ms; 			 
	         ...
	         {
				  //EVENT_INTERRUPT
                 if( cur_timer_or_counter_ctrl_ptr->elapsed_count_per_timeout + 1 == cur_timer_or_counter_ctrl_ptr->max_count_per_timeout)
                 {     
                	 //	commented due to timer running infinitely bug, without interrupting on TC match on Match_Register_id and process ISR_Timer_1()	    
				     /* 
					  *tmr_mcr_ptr[timer_ch_id] &= ~(1 << (1 + (3 * cur_timer_or_counter_ctrl_ptr->match_or_capture_id)));
