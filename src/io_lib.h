...
extern uint32_t volatile *const io_port_set_ptr[NUM_PORT_CHS];
extern uint32_t volatile *const io_port_clr_ptr[NUM_PORT_CHS];
extern uint32_t volatile *const io_port_pin_ptr[NUM_PORT_CHS];
extern uint32_t default_port_mask[NUM_PORT_CHS]; 
   
/* -------------------- public function declaration --------------------------------------- */
uint8_t IO_Channels_Func_Set(const io_config_t *const io_config_ptr);
uint8_t IO_Channel_Read(const uint8_t io_ch, uint8_t *const io_ch_read_data_ptr);
