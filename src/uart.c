...
BUGS           :              
-*------------------------------------------------------------*/
uint8_t UART_Init(const uint8_t uart_ch_id, const uint8_t init_mode)
{
  stream_t *stdout_ptr = &stdout_vt;
	uart_ctrl_t *cur_uart_ctrl_ptr;
	uint8_t uart_io_ch, ret_status, temp_data;
     ...
     *uart_fcr_ptr[uart_ch_id] = 0x03;  //FIFO Enable 
     stdout_ptr->stream_buffer_type = STREAM_BUFFER_OUTPUT;
     Stream_Flush(stdout_ptr);
	  break;  
    default:
        system_status_flag = ERR_UART_INIT;
	   	  Error_Or_Warning_UART_Proc(TRACE_UART_CH_ID, "10.01.06", ERROR_OCCURED);
	   	  return system_status_flag;	  
	}
	return SUCCESS;
}
...
BUGS           :              
-*------------------------------------------------------------*/
uint8_t UART_Receive_Char(const uint8_t uart_ch_id, void *const dev_input_ctrl_para_ptr )
{
	uart_ctrl_t *cur_uart_ctrl_ptr;
	uart_status_t *cur_uart_status_ptr;
	...
	uint8_t uart_rcvd_status, ret_status;
	
	if(rcvd_dev_input_ctrl_para_ptr == NULL_PTR)
	{
		system_status_flag = ERR_NULL_PTR;
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t UART_Disable(const uint8_t uart_ch_id)
{
	 uart_ctrl_t *cur_uart_ctrl_ptr = NULL_PTR;
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t UART_Enable(const uint8_t uart_ch_id)
{
      uart_ctrl_t *cur_uart_ctrl_ptr = NULL_PTR;
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t Disable_All_UARTs(void)
{
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t UART_Allow_Access(const uint8_t uart_ch_id)
{
	 uart_ctrl_t *cur_uart_ctrl_ptr = NULL_PTR;
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t UART_No_Access(const uint8_t uart_ch_id)
{
	 uart_ctrl_t *cur_uart_ctrl_ptr = NULL_PTR;
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t UART_DeInit(const uint8_t uart_ch_id, const uint8_t deinit_mode)
{
  stream_t *stdout_ptr = &stdout_vt;
    ...
    case DEV_DEINIT_OPER:
        Stream_Flush(stdout_ptr);
        stdout_ptr->stream_buffer_type = STREAM_BUFFER_INVALID;
    break;  
	  default:
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t UART_Transmit_Enable_Ctrl(const uint8_t uart_ch_id, const uint8_t transmit_enable_flag)
{
	 uart_ctrl_t *cur_uart_ctrl_ptr = NULL_PTR;
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t  UART_Receive_Enable_Ctrl(const uint8_t uart_ch_id, const uint8_t receive_enable_flag)
{
	uart_ctrl_t *cur_uart_ctrl_ptr = NULL_PTR;
...
BUGS           :   
-*------------------------------------------------------------*/
uint8_t UART_Transmit(const uint8_t uart_ch_id, const void *const uart_transmit_ctrl_para_ptr)
{
	data_write_ctrl_ptr_t *uart_data_transmit_ptr;	
    uart_ctrl_t *cur_uart_ctrl_ptr = NULL_PTR;
	uart_status_t *cur_uart_status_ptr = NULL_PTR;
    ...
    if(uart_transmit_ctrl_para_ptr == NULL_PTR)
	{
		system_status_flag = ERR_NULL_PTR;
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t UART_Transmit_Ctrl_Proc(const uint8_t uart_ch_id, const uint8_t uart_ctrl_format, const char uart_transmit_char )
{	
    uint8_t ret_status;
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t UART_Reset_Status(const uint8_t uart_ch_id, const uint8_t uart_reset_status_type)
{	
    uart_status_t *cur_uart_status_ptr = NULL_PTR;
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t UART_Set_Status(const uint8_t uart_ch_id, const uint8_t uart_set_status_type, const uint8_t set_data )
{	
    uart_status_t *cur_uart_status_ptr = NULL_PTR;	
		...
		{ 
			...
			case 'd' : 
			   ...
			   { 
			    ...
			    if((ret_status = Put_Str(Num_Convert(print_num, 10))) != EOF)
				{
			...
			case 'o': 
			   ...
			   if((ret_status = Put_Str(Num_Convert(print_num, 8))) != EOF)
			   {
				   num_chars_printed += ret_status;
			   }
			break;						
			...
			case 'x':
				...
				if((ret_status = Put_Str(Num_Convert(print_num, 16))) != EOF)
				{
					 num_chars_printed += ret_status;
				}					
			break;
...
char Put_Char(const char to_disp_char)
{
	...
	if(stdout_ptr->num_chars_stream_buffer + 1 < STREAM_BUFFER_LEN )
	{
	   stdout_ptr->stream_buffer[stdout_ptr->num_chars_stream_buffer] = displayed_char;	
	   ++stdout_ptr->num_chars_stream_buffer;
       stdout_ptr->stream_buffer[stdout_ptr->num_chars_stream_buffer] = NULL_CHAR;
	}
...
int16_t Put_Str(const char *const to_disp_str)
{
  ...
  dev_id_t uart_dev_id;
	...
	if(stdout_ptr->num_chars_stream_buffer + num_chars_displayed < STREAM_BUFFER_LEN )
	{
	     strcpy((stdout_ptr->stream_buffer + stdout_ptr->num_chars_stream_buffer),to_disp_str);  			
         stdout_ptr->num_chars_stream_buffer += ret_status;
         stdout_ptr->stream_buffer[stdout_ptr->num_chars_stream_buffer]  = NEW_LINE_CHAR;
         ++stdout_ptr->num_chars_stream_buffer;	   
	     stdout_ptr->stream_buffer[stdout_ptr->num_chars_stream_buffer] = NULL_CHAR;
	}
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t Error_Or_Warning_UART_Proc(const uint8_t uart_ch_id, const char *const error_trace_str, const uint8_t warn_or_error_format)
{
  uint8_t ret_status;
