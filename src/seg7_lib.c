...
uint8_t Seg7_Write(const uint8_t seg7_ch_id, const void *const seg7_rcv_disp_num_ptr)
{
	...
	uint8_t cc_digit[] = {0x3F, 0x06, 0x5B, 0x4F, 0x66, 0x6D, 0x7D, 0x07, 0x7F, 0x6F, 0x3E ,0x5E, 0x40, 0x79 }, ret_status = SUCCESS, base;
	...
	{
		 if((ret_status = IO_Channel_Write(cur_seg7_ctrl_ptr->io_ch_unit_digit + i, !(cur_seg7_ctrl_ptr->digit_place_active_state) )) != SUCCESS)
		 {						 
	...
	case DATA_TYPE_IN_DECIMAL:	
       for(repeat = 0; repeat < REQ_CNT_SEG7_REPEAT_NUM; ++repeat)
       {
		   ...
		   for(place_val = cur_seg7_ctrl_ptr->num_digits - 1; place_val >= 0; --place_val)
		  {
		     ...
		     if((ret_status = IO_Channel_Write(cur_seg7_ctrl_ptr->io_ch_unit_digit + place_val, cur_seg7_ctrl_ptr->digit_place_active_state )) != SUCCESS)
		     {						 
			      system_status_flag = ERR_SEG7_NO_WRITE;
	  ...
	  case DATA_TYPE_IN_APPL_DEFINED:
	     for(repeat = 0; repeat < REQ_CNT_SEG7_REPEAT_NUM; ++repeat)
         {
           for(place_val = cur_seg7_ctrl_ptr->num_digits - 1; place_val >= 0; --place_val)
		  {
			 if(retrieved_seg7_data_ptr[place_val] >= '0' && retrieved_seg7_data_ptr[place_val] <= '9')
			 {
				 place_digit[place_val] = retrieved_seg7_data_ptr[place_val] - NUM_0_CHAR;
			 }
			 else
			 {
				place_digit[place_val] = (retrieved_seg7_data_ptr[place_val] - SEG7_START_NON_DIGIT_CODE) + SEG7_MAX_NUM_HEXA_DIGITS;
				
				if(place_digit[place_val] >= sizeof(cc_digit))
				{
					system_status_flag = ERR_FORMAT_INVALID;
				    Error_or_Warning_Proc("06.01.24", ERROR_OCCURED, system_status_flag);
                    return system_status_flag;
				}
			 }
			 if(cur_seg7_ctrl_ptr->seg7_type == SEG7_COMMON_CATHODE)
		     ...
		     {						 
			     system_status_flag = ERR_SEG7_NO_WRITE;
				 Error_or_Warning_Proc("06.01.26", ERROR_OCCURED, system_status_flag);
			     return system_status_flag;  
		     } 
		     ...
		     {
			    system_status_flag = ERR_SEG7_NO_WRITE;
				Error_or_Warning_Proc("06.01.27", ERROR_OCCURED, system_status_flag);
			    return system_status_flag;
		     }
		    ...
		    {						 
			   system_status_flag = ERR_SEG7_NO_WRITE;
			   Error_or_Warning_Proc("06.01.28", ERROR_OCCURED, system_status_flag);
			   return system_status_flag;  
		    }
		 } 		
	  }
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t Seg7_Init(const uint8_t seg7_ch_id, const uint8_t init_mode)
{
	 seg7_ctrl_t *cur_seg7_ctrl_ptr = NULL_PTR;
...
Bugs           :  
-*------------------------------------------------------------*/
uint8_t Seg7_DeInit(const uint8_t seg7_ch_id, const uint8_t deinit_mode)
{
